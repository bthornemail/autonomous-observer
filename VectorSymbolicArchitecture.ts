...
export class VectorSymbolicArchitecture {
  ...
  private backend: BindingBackend = 'naive';
  // Lazy CQE engine so we don't require package unless needed
  private _cqe?: { bind: (a: number[], b: number[]) => { bound: number[] }, unbind: (bound: number[], known: number[]) => number[], normalize: (x: number[]) => number[] };

  constructor(dimension: number = 1024, backend: BindingBackend = 'naive') {
  ...
  private vectorBind(v1: HDVector, v2: HDVector): HDVector {
    if (this.backend === 'fft') {
      const bound = this.cqe().bind(v1.dimensions, v2.dimensions).bound;
      return {
        dimensions: this.cqe().normalize(bound),
        semanticBinding: `bind(${v1.semanticBinding}, ${v2.semanticBinding})`,
        phiRatio: this.phi
      };
    }
    // naive O(N^2) circular convolution
  ...
  unbindVector(boundVector: HDVector, knownVector: HDVector): HDVector {
    if (this.backend === 'fft') {
      const recovered = this.cqe().unbind(boundVector.dimensions, knownVector.dimensions);
      return {
        dimensions: this.cqe().normalize(recovered),
        semanticBinding: `unbind(${boundVector.semanticBinding}, ${knownVector.semanticBinding})`,
        phiRatio: this.phi
      };
    }
    // Approximate unbinding using circular correlation (naive)
    const resultDimensions = new Array(this.vectorDimension).fill(0);
    for (let i = 0; i < this.vectorDimension; i++) {
      for (let j = 0; j < this.vectorDimension; j++) {
